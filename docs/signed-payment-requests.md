...
XDCID Pay Links use wallet-signed, versioned payment requests. They provide an integrity layer between a merchant's XDCID name and the payer without making XDCID a custodian or invoice host.

## Flow

1. The current XDCID owner opens `/pay`, enters the amount, token, reference, optional description, optional designated payer, and optional expiry.
2. XDCID reads the current owner from the XDCID registry on XDC Network.
...
5. Checkout verifies the signature against the current XDCID owner, checks expiry and any designated payer, and resolves the current payment address on-chain.
...
For an ordinary externally owned account (EOA), checkout recovers the EIP-712 signer and requires it to equal the current XDCID owner.

When the current XDCID owner contains contract bytecode, checkout calls the standard ERC-1271 `isValidSignature(bytes32,bytes)` function on that smart account. Verification succeeds only when the contract returns the ERC-1271 magic value `0x1626ba7e`. A revert, malformed response, wrong magic value, or unavailable RPC response fails closed and blocks payment.
...
Changing any signed field invalidates verification. If the XDCID name is transferred after signing, checkout blocks the request because the old signer is no longer the current owner.
...
- Payment goes directly from the payer wallet to the address currently resolved by the XDCID name.
