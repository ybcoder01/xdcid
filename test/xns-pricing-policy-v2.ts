...
describe("XNSPricingPolicyV2", function () {
  async function deployPolicy() {
    ...
    const config = {
      twoCharacterAnnualUsdMicros: 100_000_000,
  ...
  it("prices two-character names at the protected floor", async function () {
    const { policy } = await deployPolicy();
    expect(await policy.priceUsdMicros(0, 2, 1)).to.equal(100_000_000);
    expect(await policy.priceUsdMicros(0, 2, 10)).to.equal(800_000_000);
    expect(await policy.priceUsdMicros(1, 2, 10)).to.equal(800_000_000);
  });
  ...
  it("never permits the two-character price below $100", async function () {
    const { policy, config } = await deployPolicy();
    await expect(
      policy.proposeConfig({
        ...config,
        twoCharacterAnnualUsdMicros: 99_999_999,
      }),
    ).to.be.revertedWithCustomError(policy, "InvalidConfig");
  });

  it("allows delayed admin price changes at or above the floor", async function () {
