...
/// @notice Pricing policy for the next modular XDCID registrar stack.
/// @dev This policy is intentionally not compatible with the currently active
/// registrar's PricingConfig ABI and must only be connected to Registrar v2.
contract XNSPricingPolicyV2 is Ownable {
    ...
    struct PricingConfig {
        uint64 twoCharacterAnnualUsdMicros;
        ...
        uint64 premiumSubdomainAnnualUsdMicros;
    ...
    function _validateConfig(PricingConfig memory target) internal pure {
        if (
            ...
            target.premiumSubdomainAnnualUsdMicros == 0 ||
